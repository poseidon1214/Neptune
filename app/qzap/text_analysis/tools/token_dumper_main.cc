...
int main(int argc, char** argv) {
  ...
  while (std::getline(fin, line)) {
    ...
    int cnt = 0;
    for (size_t i = 0; i < document.bow_token_size(); ++i) {
      for (int j = 0; j < document.bow_token(i).tf(); ++j) {
        token_text += document.bow_token(i).text() + "\t";
        ++cnt;
      }
    }
    if (cnt > 1) {
      TrimString(&token_text);
      fout << token_text << std::endl;
    }
  }
