...
class SparseLDAHillClimberTest : public ::testing::Test {
 public:
  SparseLDAHillClimberTest() : model_(0) {}

  virtual ~SparseLDAHillClimberTest() {}
  ...
  Model model_;
  Vocabulary vocab_;
  scoped_ptr<SparseLDAHillClimber> sampler_;
};  // class SparseLDAHillClimberTest

TEST_F(SparseLDAHillClimberTest, NotCache_Interpret) {
  sampler_.reset(new SparseLDAHillClimber(model_, vocab_, 0, 10));
  TestInterpret();
}

TEST_F(SparseLDAHillClimberTest, Cache_Interpret) {
  sampler_.reset(new SparseLDAHillClimber(model_, vocab_, 5, 10));
  TestInterpret();
}
