...
#include "app/qzap/common/utility/interval.h"
...
class IntervalTest : public testing::Test {
 protected:
  ...
  void TestIntersect(const Interval<int64_t> &i1,
    ...
    Interval<int64_t> i;
    i.CopyFrom(i1);
    EXPECT_TRUE(i.IntersectWith(i2) == changes_i1 &&
                i.Equals(result));
    i.CopyFrom(i2);
    EXPECT_TRUE(i.IntersectWith(i1) == changes_i2 &&
                i.Equals(result));
  }
};
...
TEST_F(IntervalTest, CoveringOps) {
  ...
  // Center:
  ASSERT_EQ(d.Center(), 150);
  ASSERT_EQ(d1.Center(), 25);
  ASSERT_EQ(d2.Center(), 80);
  ASSERT_EQ(d3.Center(), 145);
  ASSERT_EQ(d4.Center(), 200);
  ASSERT_EQ(d5.Center(), 260);
  ASSERT_EQ(d6.Center(), 125);
  ASSERT_EQ(d7.Center(), 175);
  ASSERT_EQ(d8.Center(), 150);
  ...
  // Difference:
  std::vector<Interval<int64_t>*> diff;

  EXPECT_TRUE(!d.Difference(empty, &diff));
  EXPECT_EQ(1, static_cast<int>(diff.size()));
  ...
  EXPECT_EQ(2, static_cast<int>(diff.size()));
  EXPECT_EQ(100, diff[0]->min());
