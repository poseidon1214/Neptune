...
#ifndef COMMON_CRYPTO_HASH_CRYPTO_HASH_BASE_H_
#define COMMON_CRYPTO_HASH_CRYPTO_HASH_BASE_H_
#pragma once
...
class CryptoHashBase {
 public:
  ...
  void Final(ResultElementType(*buffer)[Size]) {
    STATIC_ASSERT(sizeof(*buffer) >= Algorithm::kDigestLength);
  ...
  std::string HexFinal() {
    ...
    Final(digest);
    return HexEncodeBuffer(digest, sizeof(digest));
  }
  ...
  static void Digest(
    ...
    STATIC_ASSERT(sizeof(*result) >= Algorithm::kDigestLength);
...
#endif  // COMMON_CRYPTO_HASH_CRYPTO_HASH_BASE_H_
