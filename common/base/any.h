...
#ifndef COMMON_BASE_ANY_H_
#define COMMON_BASE_ANY_H_
...
ValueType* AnyCast(Any* operand) {
  return operand && operand->Type() == typeid(ValueType)
      ? &static_cast<Any::Holder<ValueType>*>(  // NOLINT(runtime/casting)
          operand->m_content)->m_held
      : 0;
}

/// AnyCast operator used to extract a const ValueType pointer from an
//  const Any* will return a const pointer to the stored value.
///
/// Example Usage:
...
/// Use the RefAnyCast in these cases.
template <typename ValueType>
ValueType AnyCast(Any& operand) {  // NOLINT(runtime/references)
...
template <typename ValueType>
ValueType& RefAnyCast(Any& operand) {  // NOLINT(runtime/references)
  ValueType* result = AnyCast<ValueType>(&operand);
  if (!result) throw std::bad_cast();
  return *result;
}
...
ValueType* UnsafeAnyCast(Any* operand) {
  return &static_cast<Any::Holder<ValueType>*>(  // NOLINT(runtime/casting)
      operand->m_content)->m_held;
}
