...
class ModuleManagerImpl : public gdt::Singleton<ModuleManagerImpl> {
  struct Module {
...
void ModuleManagerImpl::InitializeAll(
  ...
  if (!m_initialized) {
    ...
    LOG(INFO) << "BuildTime: " << binary_version::kBuildTime;
    LOG(INFO) << "BuildType: " << binary_version::kBuildType;
    LOG(INFO) << "BuilderName: " << binary_version::kBuilderName;
    LOG(INFO) << "HostName: " << binary_version::kHostName;
    LOG(INFO) << "Compiler: " << binary_version::kCompiler;
    LOG(INFO) << "Version:";
    for (int i = 0; i < binary_version::kSvnInfoCount; ++i) {
      LOG(INFO) << i << ":\n" << binary_version::kSvnInfo[i];
    }
    m_initialized = true;
  } else {
