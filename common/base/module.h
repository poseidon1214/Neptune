...
#ifndef COMMON_BASE_MODULE_H_
#define COMMON_BASE_MODULE_H_
#pragma once
...
inline void InitAllModules(int* argc, char*** argv, bool remove_flags = true) {
  ::gdt::internal::ModuleManager::InitializeAll(argc, argv, remove_flags);
}
...
::gdt::internal::ModuleRegisterer name##_module_registerer( \
...
namespace modules { \
PP_DISALLOW_IN_HEADER_FILE() \
::gdt::internal::ModuleDtorRegisterer name##_module_dtor_registerer( \
  __FILE__, __LINE__, #name, &name##_ModuleDtor); \
} \
\
static void name##_DoModuleDtor(); \
static void name##_ModuleDtor() { \
  ::gdt::internal::ModuleManager::LogModuleDtor(#name); \
  name##_DoModuleDtor(); \
} \
static void name##_DoModuleDtor()
...
namespace modules { \
PP_DISALLOW_IN_HEADER_FILE() \
extern ::gdt::internal::ModuleRegisterer dep_name##_module_registerer; \
static ::gdt::internal::ModuleDependencyRegisterer using_module_##dep_name( \
  __FILE__, __LINE__, \
  THIS_MODULE_NAME, #dep_name, &dep_name##_module_registerer); \
}

// make it easy to use for end users
using ::gdt::InitAllModules;
using ::gdt::InitAllModulesAndTest;

#endif  // COMMON_BASE_MODULE_H_
