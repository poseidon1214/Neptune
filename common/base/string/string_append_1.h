...
void StringAppend(std::string* str, const T& value) {
  ...
  // disable them
  STATIC_ASSERT((!std::tr1::is_same<T, signed char>::value),
                "signed char is not allowed");
  STATIC_ASSERT((!std::tr1::is_same<T, unsigned char>::value),
                "unsigned char is not allowed");
  details::StringAppendEnum<std::tr1::is_enum<T>::value>::Append(str, value);
}
