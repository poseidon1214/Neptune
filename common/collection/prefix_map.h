...
// Author: CHEN Pheng <phongchen@tencent.com>
// Author: YU Shizhan <stanyu@tencent.com>

#ifndef COMMON_COLLECTION_PREFIX_MAP_H_
#define COMMON_COLLECTION_PREFIX_MAP_H_
...
class PrefixMap {
 ...
 public:
  ...
  int FindMatchLength(const StringPiece& str, MappedType* result) const {
    if (!m_length_map.empty()) {
      typename LengthMapType::const_iterator i =
          m_length_map.lower_bound(str.size());
 ...
 private:
  static uint64_t GetDigest(const StringPiece& str) {
    return MD5::Digest64(str.data(), str.size());
  }

  // Internal insert
  bool InternalInsert(const StringPiece& str, const MappedType& value,
                      bool inclusive) {
    uint64_t digest = GetDigest(str);
...
#endif  // COMMON_COLLECTION_PREFIX_MAP_H_
