...
static void ParseSocketAddressInetTest() {
  ...
  EXPECT_TRUE(address.Parse("67.220.91.0017:65535"));
  EXPECT_EQ("67.220.91.15:65535", address.ToString());
...
TEST(SocketAddress, InetCompare) {
  SocketAddressInet address1("192.168.1.1:1000");
  SocketAddressInet address2("192.168.2.1:1000");
  EXPECT_EQ(address1, address1);
  EXPECT_NE(address1, address2);
}

TEST(SocketAddressDeathTest, Inet4BadAddress) {
  ...
  for (size_t i = 0; i < ARRAY_SIZE(bad_address); ++i) {
    EXPECT_DEATH(SocketAddressInet4 a(bad_address[i]), "Invalid IPv4");
    EXPECT_DEATH(SocketAddressInet4 a(bad_address[i], 1000), "");
    EXPECT_DEATH(SocketAddressInet4 a((std::string(bad_address[i]))), "");
    EXPECT_DEATH(SocketAddressInet4 a(std::string(bad_address[i]), 1000), "");
  }
}
...
TEST(SocketAddress, InetOn6) {
  const char* text[] = {
    "[::]:0", "[::1]:1",
    "[2001:db8:85a3::8a2e:370:7334]:2"
  };
  for (size_t i = 0; i < sizeof(text) / sizeof(text[0]); ++i) {
    SocketAddressInet address(text[i]);
    EXPECT_EQ(AF_INET6, address.Family());
    EXPECT_EQ(text[i], address.ToString());
  }
}
...
TEST(SocketAddressDeathTest, CopyFrom) {
  SocketAddressInet address1("192.168.1.1:1000");
  ...
  EXPECT_DEATH({ address1 = address5; }, "Can't copy from");
  address3 = address4;
  EXPECT_EQ(address3.ToString(), address4.ToString());
  address3 = address2;
  EXPECT_EQ(address3.ToString(), address2.ToString());
  EXPECT_DEATH({ address3 = address5; }, "Can't copy from");
}

TEST(SocketAddressDeathTest, InetBadAddress) {
  ...
  for (size_t i = 0; i < ARRAY_SIZE(bad_address); ++i) {
    EXPECT_DEATH(SocketAddressInet a(bad_address[i]), "Invalid IPv4/6");
    EXPECT_DEATH(SocketAddressInet a((std::string(bad_address[i]))),
                 "Invalid IPv4/6");
  }
}
...
TEST(SocketAddress, Inet6) {
  ...
  EXPECT_EQ("[::]:0", address.ToString());
}

TEST(SocketAddressDeathTest, Inet6BadAddress) {
  // TODO(simonwang)
  // SocketAddressInet6 parse not implemented yet,
  static const char* bad_address[] = {
    "192.168.1.1",
  ...
  for (size_t i = 0; i < ARRAY_SIZE(bad_address); ++i) {
    EXPECT_DEATH(SocketAddressInet6 a(bad_address[i]),
                 "Invalid IPv6 socket address");
    EXPECT_DEATH(SocketAddressInet6 a((std::string(bad_address[i]))),
                 "Invalid IPv6 socket address");
  }
}
...
TEST(SocketAddress, Unix) {
  ...
  EXPECT_EQ('\0',
            reinterpret_cast<sockaddr_un*>(address3.Address())->sun_path[0]);
  EXPECT_EQ("@/data/local.socket", address3.ToString());
  address2.Clear();
  EXPECT_EQ("", address2.ToString());
}

TEST(SocketAddressDeathTest, UnixBadAddress) {
  const char* bad_address[] = {
    "abc/data/local/socket",
  };
  for (size_t i = 0; i < ARRAY_SIZE(bad_address); ++i) {
    EXPECT_DEATH(SocketAddressUnix a(bad_address[i]),
                 "Invalid unix domain socket address");
    EXPECT_DEATH(SocketAddressUnix(std::string(bad_address[i])),
                 "Invalid unix domain socket address");
  }
}
...
TEST(SocketAddress, Storage) {
  ...
  EXPECT_TRUE(storage.Parse("@/data/local.socket"));
  EXPECT_EQ('\0',
            reinterpret_cast<sockaddr_un*>(storage.Address())->sun_path[0]);
  EXPECT_EQ("@/data/local.socket", storage.ToString());
}
