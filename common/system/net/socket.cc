...
void Socket::ReportError(const char* info) const {
  LOG_IF(FATAL, m_ExceptionEnabled) << "SocketError: " << info;
}

void Socket::Attach(int handle) {
  if (handle != m_handle) {
...
bool Socket::GetLocalAddress(SocketAddress* address) const {
  ...
  if (CheckError(getsockname(m_handle, address->Address(), &length),
                 "GetLocalAddress")) {
...
bool Socket::GetPeerAddress(SocketAddress* address) const {
  socklen_t length = address->Capacity();
  if (CheckError(getpeername(m_handle, address->Address(), &length),
                 "GetPeerAddress")) {
    address->SetLength(length);
    return true;
  }
...
ListenerSocket::ListenerSocket(const SocketAddress& address, int type) :
  ...
  PCHECK(IsValid());
  PCHECK(Bind(address)) << "Can't bind to " << address.ToString();
}
...
bool ListenerSocket::Accept(
  ...
  for (;;) {
    int s = accept(Handle(), address ? address->Address() : NULL, &length);
    if (s != -1) {
      socket->Attach(s);
      if (address)
        address->SetLength(length);
...
bool DataSocket::Connect(const SocketAddress& address, int64_t timeout_ms) {
  ...
  int rv = EINTR_IGNORED(
      connect(Handle(), address.Address(), address.Length()));
...
StreamSocket::StreamSocket(int af, int protocol) {
  PCHECK(DataSocket::Create(af, SOCK_STREAM, protocol)) << "StreamSocket";
}
...
bool StreamSocket::ReceiveAll(
  ...
  bool auto_restart) {
  size_t received_size;
  return ReceiveAll(buffer, buffer_size, &received_size, timeout, flags,
                    auto_restart);
}
  ...
  *received_size = 0;
  ...
  while (buffer_size > 0) {
    ...
    if (Receive(buffer, peek_size, &n, MSG_PEEK) && n > 0) {
      ...
      if (p) {
        bool result = ReceiveAll(buffer, p - static_cast<char*>(buffer) + 1,
                                 &n);
        *received_size += n;
        return result;
      } else {
