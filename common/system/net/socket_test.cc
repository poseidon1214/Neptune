...
TEST(SocketDeathTest, ListenerSocket) {
  ...
  EXPECT_DEATH(ListenerSocket listener2(bind_address, SOCK_STREAM),
 ...
 protected:
  DatagramSocketTest() {
    ...
    m_thread.reset(new Thread(
            NewCallback(this, &DatagramSocketTest::IoThread)));
    m_thread->Start();
  }
...
class StreamSocketTest : public testing::Test {
 protected:
  StreamSocketTest() {
    ...
    m_thread.reset(new Thread(NewCallback(this, &StreamSocketTest::IoThread)));
    m_thread->Start();
  }
  ~StreamSocketTest() {
    m_thread->Join();
...
TEST_F(StreamSocketTest, StreamSocketReceiveLineInEqualSizeBuffer) {
  ...
  EXPECT_EQ(0, memcmp(sent.data(), buffer, 6));
}
