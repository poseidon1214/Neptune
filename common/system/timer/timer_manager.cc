...
uint64_t TimerManager::AddOneshotTimer(
    int64_t interval,
    CallbackClosure* closure) {
  assert(closure != NULL);
  assert(!closure->IsRepeatable());
  return AddTimer(interval, false, closure);
}

uint64_t TimerManager::AddPeriodTimer(
    int64_t interval,
    CallbackClosure* closure) {
  assert(closure != NULL);
  assert(closure->IsRepeatable());
...
void TimerManager::PushNextTimeout(
    uint64_t id,
    int64_t interval,
    uint8_t revision) {
...
void TimerManager::SetNextTimeout(
    uint64_t id,
    int64_t interval,
    uint8_t revision) {
  assert(m_mutex.IsLocked());
  ...
  for (;;) {
    {
      ...
      if (GetLatestTimeout(&time)) {
        m_cond.TimedWait(&m_mutex, static_cast<int>(time));
      } else {
        m_cond.Wait(&m_mutex);
      }
    }
