...
struct UriParserByteSets {
 ...
 private:
  UriParserByteSets():
    ...
    reg_name(unreserved | "$,,:@&=+"),
    // "|", "{" and "}" are not valid in RFC3986,
    // but has been used in gdt frontend protocol, admit it.
...
class UriParser {
 ...
 private:  // in RFC 3986 Appendix A.  Collected ABNF for URI order
  ...
  // userinfo      = *( unreserved | escaped | ";" | ":" | "&" | "=" | "+" | "$" | "," )  // NOLINT
  bool match_userinfo() {
 ...
 private:  // helpers
  ...
  bool match_domainlabel() {
    ...
    if (match_alphanum()) {
      Result r1(this);
      for (;;) {
        while (match_literal('-')) {  // || match_literal('_')) // allow _ in domain?  // NOLINT
        }
        if (match_alphanum())
          r1 = true;
        else
  ...
  bool match_query_or_fragment() {
    while (match_byteset(m_byte_sets.query_or_fragment) ||
           match_pct_encoded()) {
    }
    return true;
  }
  ...
  template <size_t N>
  bool match_literal(const char(&l)[N]) {  // NOLINT
...
bool URI::WriteToBuffer(char* buffer, size_t buffer_size,
...
static void remove_dot_segments(const std::string& path, std::string* result) {
  ...
  while (input_length > 0) {
    ...
    // rule B
    else if (input_length >= 3 && memeql(input_buffer, "/./", 3)) {  // NOLINT
      input_buffer += 2;
      input_length -= 2;
      *input_buffer = '/';
    } else if (input_length == 2 && memeql(input_buffer, "/.", 2)) {
    ...
    // rule C
    else if (input_length >= 4 && memeql(input_buffer, "/../", 4)) {  // NOLINT
      input_buffer += 3;
      input_length -= 3;
      *input_buffer = '/';
    ...
    // rule D
    else if (input_length == 1 && *input_buffer == '.') {  // NOLINT
      ++input_buffer;
      --input_length;
    } else if (input_length == 2 && memeql(input_buffer, "..", 2)) {
      input_buffer += 2;
    ...
    // rule E
      ...
      char* p = reinterpret_cast<char*>(memchr(input_buffer + 1, '/',
                                        input_length - 1));
      if (p) {
