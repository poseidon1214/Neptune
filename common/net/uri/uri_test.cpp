...
TEST(URI, Parse) {
  std::string uristr =
    "http://www.baidu.com/s?tn=monline_dg&bs=DVLOG&f=8&wd=glog+DVLOG#fragment";
  URI uri;
...
TEST(URI, AllowGdtExtended) {
  const std::string str =
      "http://q.i.gdt.qq.com/gdt_view?uin=527995216|2|3&"
      "is_array=function%20()%20{%20return%20instance}";
  URI uri;
  ASSERT_TRUE(uri.Parse(str));
}
...
TEST(URI, Set) {
  ...
  EXPECT_EQ("http://username:password@127.0.0.1:8080/search?q=abc#fragment",
            uri.ToString());
}
...
TEST(URI, ParseAuthority) {
  ...
  EXPECT_EQ("http://administrator@10.12.22.200:8099/s?tn=monline_dg&bs=DVLOG",
            uri.ToString());
