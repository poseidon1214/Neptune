...
class URI {
 ...
 public: // Attributes
  ...
  // UserInfo
  bool HasUserInfo() const {
    return m_has_authority && m_authority.HasUserInfo();
  }
  const std::string& UserInfo() const {
    assert(m_has_authority);
  ...
  // Port
  bool HasPort() const {
    return m_has_authority && m_authority.HasPort();
  }
  const std::string& Port() const {
    assert(m_has_authority);
...
inline void swap(gdt::net::UriAuthority& x, gdt::net::UriAuthority& y) {
  x.Swap(&y);
}

inline void swap(gdt::net::URI& x, gdt::net::URI& y) {
  x.Swap(&y);
}
