...
#ifndef COMMON_NET_HTTP_HTTP_RESPONSE_H_
#define COMMON_NET_HTTP_HTTP_RESPONSE_H_
#pragma once
...
class HttpResponse: public HttpMessage {
 public:
  ...
  void FillWithHtmlPage(int code,
                        const StringPiece& title = "",
                        const StringPiece& body = "");

 public:
  HttpResponse() : m_status(0) {}
  ~HttpResponse() {}
  virtual void Reset();
  ...
  void AppendBodyChunk(StringPiece chunk);
...
template <>
inline void swap(gdt::HttpResponse& lhs, gdt::HttpResponse& rhs) {
  lhs.Swap(&rhs);
}

}  // namespace std

#endif  // COMMON_NET_HTTP_HTTP_RESPONSE_H_
